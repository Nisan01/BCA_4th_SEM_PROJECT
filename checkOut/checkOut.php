...
// Encode the cart data to store as checkout_product_id
$checkout_product_id = json_encode($cart);

// Check if an order with the same checkout_product_id already exists for this user
$check_order_query = "SELECT * FROM orders WHERE checkout_product_id = '$checkout_product_id' AND id = '$user_id'";
...
$insert_order_query = "INSERT INTO orders (order_id, id, Fullname, Email, total_price, payment_status, billing_info, created_at, checkout_product_id) 
                       VALUES ('$order_id', '$user_id', '$user_name', '$user_email', $total, 'Pending', '', NOW(), '$checkout_product_id')";
